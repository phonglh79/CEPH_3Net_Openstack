...
ssh -t ceph2 sudo mkdir -p /var/lib/ceph/mon/ceph-$HOST2 /tmp/$HOST2
ssh -t ceph3 sudo mkdir -p /var/lib/ceph/mon/ceph-$HOST3 /tmp/$HOST3

#Tao keyring cho Ceph Cluster
echo "############ Tao keyring cho Ceph Cluster ############"
ssh -t ceph2 sudo ceph auth get mon. -o /tmp/$HOST2/monkeyring
ssh -t ceph3 sudo ceph auth get mon. -o /tmp/$HOST3/monkeyring

#Lay Monitor map
echo "############ Lay Monitor map ############"
ssh -t ceph2 sudo ceph mon getmap -o /tmp/$HOST2/monmap
ssh -t ceph3 sudo ceph mon getmap -o /tmp/$HOST3/monmap

#Tao Monitor daemon
echo "############ Tao Monitor daemon ############"
ssh -t ceph2 sudo ceph-mon -i $HOST2 --mkfs --monmap /tmp/$HOST2/monmap --keyring /tmp/$HOST2/monkeyring
ssh -t ceph3 sudo ceph-mon -i $HOST3 --mkfs --monmap /tmp/$HOST3/monmap --keyring /tmp/$HOST3/monkeyring

#Add monitor moi vao cluster
echo "############ Add monitor moi vao cluster ############"
ssh -t ceph2 sudo touch /var/lib/ceph/mon/ceph-ceph2/sysvinit
ssh -t ceph2 sudo /etc/init.d/ceph start mon.ceph2
ssh -t ceph3 sudo touch /var/lib/ceph/mon/ceph-ceph3/sysvinit
ssh -t ceph3 sudo /etc/init.d/ceph start mon.ceph3
